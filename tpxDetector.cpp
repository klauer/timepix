...
: ADDriver(portName, 1, (int)NUM_TPX_PARAMS, maxBuffers, maxMemory, 0, 0, ASYN_CANBLOCK, 1, priority, stackSize), writeRawDataFlag(false), lastPreviewUpdate(0), rawDataFile(NULL)
{
...
void tpxDetector::acquireTask(void)
{
    ...
    //epicsTimeStamp testTime1, testTime2;
 ...
 //   while(!driverReady);
    ...
    while (1) {
        ...
        if (!acquire) {
            ...
            if(driverReady) this->resetDetector();
            this->unlock();
...
//////////////////////////////////////////////////////////////////////////////////
        ...
        if (trigmode==TPX_EXTERNAL_TRIGGER) 
        {
            ...
            printf("---- EXTERNAL TRIGGER MODE ----\n");
        //	Trigger controlled acquisition			
            // ----------------------------------------------------------------------------------------------			        
            getIntegerParam(ADNumImagesCounter, &numImagesCounter);
            printf("ext triggering; num images counter=%d\n", numImagesCounter);
            ...
            while( 1 ) {	
                if ((*relaxd->newFrame)(ids[uiDevIp_], true, true)) 
                {
                    ...
                    if(writeRawDataFlag) {
                        ...
                        printf("**** Raw data to write, but writeRawDataFlag=false\n");
                    }
#else
                ...
                if (estatus==epicsEventWaitOK) {
                    ...
                    asynPrint(this->pasynUserSelf, ASYN_TRACE_FLOW,
                              "%s:%s: acquisition broken\n", driverName, functionName);
                    printf("acquisition broken?\n");
                    callParamCallbacks();
                    break;
                }
            }	
  //          if (estatus==epicsEventWaitOK) continue;
      ...
      //          epicsThreadSleep(60.0);
		...
		//	if(!writeRawDataFlag || isTimeForPreviewUpdate())
...
void tpxDetector::resetDetector (void)
{
    ...
    if (driverReady)
    {
        // for (int reset_tries=0; reset_tries < 5; reset_tries++) {
